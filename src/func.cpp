...
bool fallDetXAlgorithm(const std::vector<float>& kps)
{
    ...
    if (L_knee_confi > 0.0f && R_knee_confi > 0.0f && L_hip_confi > 0.0f && R_hip_confi > 0.0f)
    {
        ...
        float th1 = 30.0f;  // 最小角度阈值
        float th2 = 70.0f;  // 最大角度阈值
    ...
    if (L_shoulder_confi > 0.0f && R_shoulder_confi > 0.0f && L_hip_confi > 0.0f && R_hip_confi > 0.0f &&
        L_knee_confi > 0.0f && R_knee_confi > 0.0f && L_ankle_confi > 0.0f && R_ankle_confi > 0.0f)
    {
        // 计算向量 v1 和 v2
        ...
        // 设定角度阈值 th3 和 th4
        float th3 = 70.0f;  // 肩、髋、膝夹角阈值
        float th4 = 30.0f;  // 髋、膝、脚踝夹角阈值
